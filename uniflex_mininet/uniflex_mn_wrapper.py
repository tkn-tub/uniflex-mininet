...
class UniFlexNode(object):
    """A UniFlex node is either a UniFlex agent or controller."""
    ...
    def __init__(self, network_node, config, logfile, verbose=False):
        ...
        if logfile is None:
            logfile = '/tmp/uniflex_' + network_node.name + '.log'
        else:
            self.logfile = logfile

        self.verbose = verbose
        self.script = 'uniflex-agent'
        print('Starting UniFlex agent with config: %s and logfile: %s' % (self.config, self.logfile))
